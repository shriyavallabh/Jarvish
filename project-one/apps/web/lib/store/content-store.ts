...
export const useContentStore = create<ContentState>((set, get) => ({
  ...
  checkCompliance: async (request: ComplianceCheckRequest) => {
    ...
    try {
      // Map request fields to match backend API
      const apiRequest = {
        content: (request as any).text || (request as any).content,
        contentType: (request as any).contentType || 'whatsapp',
        language: (request as any).language || 'en'
      };
