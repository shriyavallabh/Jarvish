...
export function useComplianceCheck(
      ...
      try {
        const response = await api.post<ComplianceCheckResponse>(
          '/compliance/check',
          {
            ...
            contentType: type === 'market_update' ? 'whatsapp' : 
                        type === 'educational' ? 'status' : 
                        type === 'promotional' ? 'linkedin' : 'whatsapp',
            language,
          }
        );
  ...
  // Map issues to flags for backward compatibility
  const flags = result?.issues?.map(issue => ({
    type: issue.severity === 'high' ? 'error' as const : 
          issue.severity === 'medium' ? 'warning' as const : 'info' as const,
  ...
  return {
    ...
    approved: result?.isCompliant || false,
